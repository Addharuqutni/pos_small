import { pgSchema, pgEnum, pgTable, uuid, bigint, text, integer, varchar, timestamp, bigserial, json, boolean, jsonb, customType, inet, smallint, index, uniqueIndex, foreignKey, primaryKey, unique, check, doublePrecision, numeric } from "drizzle-orm/pg-core"
...
export const role = pgEnum("role", ["owner", "admin", "cashier"])
export const saleStatus = pgEnum("sale_status", ["paid", "void", "refunded", "partial_refunded"])
export const shiftStatus = pgEnum("shift_status", ["open", "closed"])
export const stockMovementType = pgEnum("stock_movement_type", ["sale", "adjustment", "return", "restock", "refund"])
...
export const auditLogs = pgTable("audit_logs", {
	...
	index("audit_logs_actor_user_id_idx").using("btree", table.actorUserId.asc().nullsLast()),
	index("audit_logs_created_at_idx").using("btree", table.createdAt.asc().nullsLast()),
	index("audit_logs_entity_idx").using("btree", table.entityType.asc().nullsLast(), table.entityId.asc().nullsLast()),
]);
...
export const categories = pgTable("categories", {
	...
	index("categories_is_active_idx").using("btree", table.isActive.asc().nullsLast()),
]);
...
export const payments = pgTable("payments", {
	...
	index("payments_sale_id_idx").using("btree", table.saleId.asc().nullsLast()),
...
export const products = pgTable("products", {
	...
	uniqueIndex("products_barcode_idx").using("btree", table.barcode.asc().nullsLast()).where(sql`((barcode IS NOT NULL) AND ((barcode)::text <> ''::text))`),
	index("products_is_active_idx").using("btree", table.isActive.asc().nullsLast()),
	index("products_name_idx").using("btree", table.name.asc().nullsLast()),
	uniqueIndex("products_sku_idx").using("btree", table.sku.asc().nullsLast()).where(sql`((sku IS NOT NULL) AND ((sku)::text <> ''::text))`),
check("products_cost_price_check", sql`(cost_price >= 0)`),check("products_price_check", sql`(price >= 0)`),]);
...
export const refundItems = pgTable("refund_items", {
	...
	index("refund_items_refund_id_idx").using("btree", table.refundId.asc().nullsLast()),
check("refund_items_amount_check", sql`(amount >= 0)`),check("refund_items_qty_check", sql`(qty > 0)`),]);
...
export const refunds = pgTable("refunds", {
	...
	index("refunds_sale_id_idx").using("btree", table.saleId.asc().nullsLast()),
]);
...
export const saleItems = pgTable("sale_items", {
	...
	index("sale_items_sale_id_idx").using("btree", table.saleId.asc().nullsLast()),
check("sale_items_discount_check", sql`(discount >= 0)`),check("sale_items_price_check", sql`(price >= 0)`),check("sale_items_qty_check", sql`(qty > 0)`),]);
...
export const sales = pgTable("sales", {
	...
	changeTotal: integer("change_total").default(0).notNull(),
	status: saleStatus().default("paid").notNull(),
	createdAt: timestamp("created_at", { withTimezone: true }).default(sql`now()`).notNull(),
	updatedAt: timestamp("updated_at", { withTimezone: true }).default(sql`now()`).notNull(),
}, (table) => [
	index("sales_cashier_id_idx").using("btree", table.cashierId.asc().nullsLast()),
	index("sales_created_at_idx").using("btree", table.createdAt.asc().nullsLast()),
	uniqueIndex("sales_invoice_no_idx").using("btree", table.invoiceNo.asc().nullsLast()),
	index("sales_shift_id_idx").using("btree", table.shiftId.asc().nullsLast()),
	index("sales_status_idx").using("btree", table.status.asc().nullsLast()),
check("sales_grand_total_check", sql`(grand_total >= 0)`),check("sales_subtotal_check", sql`(subtotal >= 0)`),]);
...
export const settings = pgTable.withRLS("settings", {
	id: uuid().defaultRandom().primaryKey(),
	storeName: varchar("store_name", { length: 255 }).default("Toko Saya").notNull(),
	storeAddress: text("store_address").default("").notNull(),
	storePhone: varchar("store_phone", { length: 50 }).default("").notNull(),
	receiptFooter: text("receipt_footer").default("Terima kasih atas kunjungan Anda").notNull(),
	...
	allowNegativeStockDefault: boolean("allow_negative_stock_default").default(false).notNull(),
});
...
export const shifts = pgTable("shifts", {
	...
	difference: integer(),
	status: shiftStatus().default("open").notNull(),
}, (table) => [
	index("shifts_cashier_id_idx").using("btree", table.cashierId.asc().nullsLast()),
	uniqueIndex("shifts_one_open_per_cashier_idx").using("btree", table.cashierId.asc().nullsLast()).where(sql`(status = 'open'::shift_status)`),
	index("shifts_status_idx").using("btree", table.status.asc().nullsLast()),
...
export const stockMovements = pgTable("stock_movements", {
	...
	index("stock_movements_created_at_idx").using("btree", table.createdAt.asc().nullsLast()),
	index("stock_movements_product_id_idx").using("btree", table.productId.asc().nullsLast()),
]);
...
export const users = pgTable("users", {
	...
	role: role().default("cashier").notNull(),
	isActive: boolean("is_active").default(true).notNull(),
